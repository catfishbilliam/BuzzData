...
async function updateLineChart(state) {
    ...
    for (const year of years) {
        ...
        if (colonyLossData) {
            // Find the data for the selected state only, ensuring case-insensitive comparison
            const stateData = colonyLossData.filter(record => record.state.toUpperCase() === state.toUpperCase());

            // Calculate the total loss for the year for the selected state
    ...
    lineChart.data.labels = years; // Set the labels to the years
    lineChart.data.datasets[0].data = annualTotals; // Update with the total losses per year
    lineChart.update();
    console.log("[INFO] Line chart updated with annual totals.");
}

// Function to update the bar chart with quarterly totals for the selected state and year
async function updateBarChart(state, year) {
    console.log(`[INFO] Updating bar chart for state: ${state}, year: ${year}`);

    // Fetch the monthly data for the selected year
    const colonyLossData = await fetchMonthlyColonyData(year);
    if (colonyLossData) {
        // Filter data specifically for the selected state, ensuring case-insensitive comparison
        const stateData = colonyLossData.filter(record => record.state.toUpperCase() === state.toUpperCase());

        if (stateData.length > 0) {
            // Extract the quarterly data for the selected state
            const quarterlyData = [
            ...
            barChart.data.datasets[0].data = quarterlyData; // Update the bar chart data
            barChart.update();
...
async function assignDataToStates(stateMeshes, year) {
    ...
    for (let mesh of stateMeshes) {
        if (mesh.metadata && mesh.metadata.stateName) {
            ...
            const coloniesRegistered = registeredColoniesData?.[year]?.[stateName]?.value;

            if (coloniesRegistered) {
                ...
                mesh.metadata.colonyCollapse = {
                    totalLoss: totalLoss,
                    coloniesRegistered: coloniesRegistered,
                ...
                logDetail(`[DEBUG] Assigned data to ${stateName}: Total Loss = ${totalLoss}, Percentage Change = ${percentageChange}%`);
            } else {
                logDetail(`[DEBUG] No registered colonies data found for ${stateName} in ${year}.`);
                mesh.metadata.colonyCollapse = {
                    totalLoss: 0,
                    coloniesRegistered: null,
                    percentageChange: 0
                };
            }
        }
...
function createScene() {
    ...
    BABYLON.SceneLoader.ImportMesh("", "/assets/", "united_states.obj", scene, function (loadedMeshes) {
